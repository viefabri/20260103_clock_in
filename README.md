...
### プロセス隠蔽による安全性
Bitwarden CLI (`bw unlock`) へのマスターパスワード受け渡しには、**標準入力 (stdin)** を使用しています。
これにより、`ps` コマンドなどのプロセスリストにパスワードが平文で表示されるリスクを回避しています。
...
- **Bitwarden CLI (`bw`)**: インストール済みであること
    - デフォルトではシステムパス (`which bw`) を使用します。
    - 異なる場所にインストールされている場合は環境変数 `BW_CLI_PATH` で指定可能です。
